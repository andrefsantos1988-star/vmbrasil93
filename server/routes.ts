...
export async function registerRoutes(
  ...
  // IMPORTANT: Excludes from value calculations: isPrepared products and categories that match isPreparedCategoryName()
  // These are prepared/mixed drinks (DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS, COPOS, DRINKS, CAIPI ICES)
  // Only regular inventory items count toward stock value - prepared items only count in sales/financial reports
  app.get("/api/stock/report", async (req, res) => {
    try {
      ...
      // Build a set of category IDs to exclude using isPreparedCategoryName function
      // This ensures DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS are always excluded
  ...
  // IMPORTANT: Excludes isPrepared products and prepared item categories using isPreparedCategoryName()
  // Prepared items like DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS are never included in shopping lists
  // These are prepared/made-to-order items that don't require inventory replenishment
  app.get("/api/stock/low-stock", async (req, res) => {
    try {
      ...
      // Build a set of category IDs to exclude using isPreparedCategoryName function
      // This ensures DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS are always excluded from shopping lists
      const excludedCategoryIds = new Set(
        categories
          .filter(c => isPreparedCategoryName(c.name))
          .map(c => c.id)
  ...
  // IMPORTANT: POST endpoint that ALWAYS excludes prepared item categories using isPreparedCategoryName()
  // Prepared items like DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS are never purchasable inventory items
  app.post("/api/stock/shopping-list", async (req, res) => {
    try {
      ...
      // Build a set of category IDs that are always excluded using isPreparedCategoryName function
      // This ensures DOSES, COPAO, DRINKS ESPECIAIS, BATIDAS, CAIPIRINHAS are NEVER in shopping lists
